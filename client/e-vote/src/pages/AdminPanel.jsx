...
const AdminPanel = () => {
    ...
    const fetchAllData = async () => {
      try {
        ...
        if (!elecRes.ok || !partyRes.ok || !candRes.ok || !notifRes.ok) {
          throw new Error(
            "Failed to fetch initial data. Is the backend running?"
          );
        }
  ...
  const handleSubmit = async (type) => {
    ...
    if (type === "party") endpoint = `${API_URL}/parties`; 
  ...
  const handleUpdate = async (type) => {
    ...
    try {
      ...
      if (type === "election")
        setElections((prev) =>
          prev.map((e) =>
            e.id === updatedItemWithId.id ? updatedItemWithId : e
          )
        );
      else if (type === "party")
        setParties((prev) =>
          prev.map((p) =>
            p.id === updatedItemWithId.id ? updatedItemWithId : p
          )
        );
      else if (type === "candidate")
        setCandidates((prev) =>
          prev.map((c) =>
            c.id === updatedItemWithId.id ? updatedItemWithId : c
          )
        );
  ...
  const sendNotification = async (id) => {
    ...
    try {
      ...
      setNotifications((prev) =>
        prev.map((n) => (n.id === id ? updatedItemWithId : n))
      );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50">
        ...
        <Modal title="Schedule New Election" onClose={() => setShowModal(null)}>
          <div className="space-y-4">
            <div>
              ...
              <input
                type="text"
                placeholder="e.g., Lok Sabha Election 2024"
                onChange={(e) =>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent text-2xl text-center"
              />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">
        ...
        <Modal title="Create Notification" onClose={() => setShowModal(null)}>
          <div className="space-y-4">
            <div>
              ...
              <input
                type="text"
                placeholder="e.g., Election Started"
                onChange={(e) =>
      ...
      {showModal?.startsWith("edit-") && (
        <Modal
          title={`Edit ${
            showModal.replace("edit-", "").charAt(0).toUpperCase() +
            showModal.slice(6)
          }`}
